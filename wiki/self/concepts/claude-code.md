...
- **Quota exhaustion.** Even when rate limits don't apply, the total token quota for a session is finite. A session that does a lot of reading (the wiki has 438 pages) can exhaust its quota before finishing.
...
- **T1 (ground pages).** Every entity page, event page, and period page was written by Claude Code reading primary sources. The 438 pages in the wiki are Claude Code products.
...
- **438 pages** in wiki/, all maintained by Claude Code
...
- **Quota exhaustion.** Reading 438 pages (each ~10KB) can exhaust the token quota.
