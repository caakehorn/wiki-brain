...
connections:
  ...
  - page: wiki/self/concepts/claude
    type: instance-of
    claim: "Claude is the LLM family's analytical member in Dan's practice, and the one whose reputation he states in his own words — 'Claude = to analyze stuff' — which makes it the concrete case the general page abstracts from."
  - page: wiki/self/concepts/claude-code
    type: instance-of
    claim: "Claude Code is the LLM with shell access, and the difference it makes is the whole of the general page's 'engine, not oracle' claim: a model that can run bin/wiki-lint is held to a mechanical check no conversational model is."
  - page: wiki/self/concepts/gemini
    type: instance-of
    claim: "Gemini is where the bootloader system was designed iteration by iteration across 3,986 logged prompts, which makes it the LLM page's evidence that the exocortex is a built artifact rather than a metaphor."
  - page: wiki/self/concepts/chatgpt
    type: instance-of
    claim: "ChatGPT is the family's first member and its cautionary case: the model that proved the bootloader concept and then, on Dan's one-line verdict, stopped being usable for it — the origin of the model-agnostic design the general page describes."
---
...
2. **The data pipeline** — the iMessage corpus (217,573 messages across 503 handles), the Facebook takeouts, the GEDCOM, the contacts exports, the YouTube watch history, the location data, the [[wiki/self/concepts/gemini|Gemini activity]] log. The bootloader tells the LLM how to access and interpret this data.
...
- **3,986 prompted entries** in the Gemini Activity log (21 MB)
- **132+ nodes** in the DANFRANK-ISMS pinned session
- **50+ tools** in the `bin/` directory, all built by Claude Code
- **473 pages** in the wiki, all maintained by LLMs
