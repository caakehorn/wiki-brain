...
connections:
  ...
  - page: wiki/self/concepts/gemini
    type: parallels
    claim: "Claude and Gemini form the core analytical pair — Claude for depth, Gemini for interaction. Tom's summary: 'Claude = to analyze stuff, gemini = interact with it.'"
...
Claude is the analytical workhorse of the wiki-brain. When Dan needs to understand something complex — a relationship, a financial thread, a psychological pattern — he uses Claude. The model has a reputation in Dan's circle for being "to analyze stuff" (as Tom put it), and that reputation is earned: Claude is the model that built the wiki, maintains it, and continues to produce the highest-altitude synthesis in the system.

## The reputation

In a 2026-03-25 exchange with Tom, Dan articulated the division of labor: "Claude = to analyze stuff." This is not a trivial distinction. Other models talk, create, interact. Claude analyzes. The difference is in the register: Claude's outputs are evidence-first, conclusion-led, and willing to state uncomfortable truths without softening. This is the model you use when you want to know what the data says, not what you want to hear.

The reputation extends to technical tasks. Dan notes: "It did really well with the Kristin chat logs. Better than GPT." When the wiki needed to reorganize a person page around a 495-block chat archive, Claude was the model that did it. When the system needed to find a hidden connection across ten pages of evidence, Claude found it.
...
- **Analytical depth.** When Tom said "Claude = to analyze stuff," he was describing a model that can read 100k+ messages and find the pattern that reorganizes a page. The wiki's synthesis layer — the junction pages and doctrine pages that represent the highest-altitude work — is built almost entirely by Claude.
...
- **T1 (ground pages).** Every entity page, event page, and period page was built by Claude reading primary sources and writing prose. The 438 pages in the wiki are, with few exceptions, Claude products.
...
- **438 pages** in the wiki, almost all built by Claude
...
Claude (via Claude Code) has a subagent dispatch system (`delegate_task`) that spawns parallel workers for multi-part tasks:
...
This is how the wiki-brain scales: not by making one agent do everything, but by spawning parallel workers for independent tasks. The limitation is rate limits: 3+ parallel agents competing for API bandwidth can trigger HTTP 429 errors that kill all of them.

## The skill system

The wiki-brain has a skill system that encodes procedural knowledge for recurring tasks. Skills are stored at `~/.hermes/skills/<name>/SKILL.md` and loaded on demand:

- **wiki-rewrite** — the wipe-and-rewrite protocol
- **wiki-brain-entry** — the meta-entry creation protocol (300+ lines minimum, NEW PR)
- **ingesting-hermes-desktop-dom** — the DOM ingestion protocol
- **hermes-agent** — the self-configuration protocol

Each skill encodes the governing docs' rules as step-by-step procedures. When Claude loads `wiki-rewrite`, it is loading the full six-phase protocol with all the governing docs' constraints encoded as steps.

## The operator answers system

The `bin/wiki-gaps` tool manages the operator answers system:
...
4. Claude integrates the answer into the page
5. `bin/wiki-gaps clear` removes the staging

This is the human-in-the-loop mechanism: the wiki asks questions, the operator answers them, and the model integrates the answers.

## The capture system

The capture system (`raw/self/captures/`) adds operator testimony to the corpus:
