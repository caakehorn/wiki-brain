---
domain: legal
page_type: summary
status: active
date_created: 2026-06-22
date_modified: 2026-06-23
date_range_start: 
sources: ["raw/self/context-core/CONTEXT_CORE_EXPANDED.md", "raw/self/dox-md/operating_manual.md", "raw/self/message-csv/imessage_7243228715_both_2025-06-03_now.csv", "raw/self/message-csv/imessage_7243228715_both_all_now.csv", "wiki/legal/463-morgantown/timeline.md", "wiki/people/alexander-jackson.md", "wiki/people/arnu.md"]
related: ["wiki/people/alexander-jackson", "wiki/people/suzanne-frank", "wiki/people/arnu", "wiki/people/john-carney", "wiki/people/felix", "wiki/self/context-core", "wiki/mind/synthesis/vertical-authority-skepticism", "wiki/timeline/periods/2025-collapse", "wiki/legal/bfs-dispute/overview", "wiki/self/facebook"]
---

# 463 Morgantown St — Overview

## Property
463 Morgantown St, Uniontown PA. Owner: [[wiki/people/alexander-jackson]] (Alexander Jackson). Dan and Suz moved in without signed lease or confirmed POA at time. Distinct from BFS building ownership.

## Occupancy / Roles
Mixed agent/tenant/caretaker for Suz. Documented misconduct in corpus. No integrated risk picture at move-in. Parallel to BFS off-books patterns per synthesis.

**Corpus Dimensions (related to 463/337 moves):**
- Suz +17243228715. Financial intermediation e.g. $750/week borrowing noted in cycles (operating_manual).
- Move timeline: Post 337 sale contingencies (buyers Jennifer J + Scott P Delverm $465k, 19 Jeffries ≤$229.9k sale, bankruptcy approval, 7-day vacate).
- Key fragments: 2026-02-10 Arnu/Felix paint + Elijah (Suz msg); 2026-03-27 "John carney thing".

## Roles Table (with new evidence)

| Role | Person/Entity | Dates / Evidence | Financial/Legal Ties | Source |
|------|---------------|------------------|----------------------|--------|
| Owner | [[wiki/people/alexander-jackson]] (Alexander Jackson) | Title holder; move-in ~2026 | Lease/POA counterparty TBD | overview, timeline, jackson page |
| Seller (337) / agent/tenant/caretaker (463) | [[wiki/people/suzanne-frank]] | 337 close ~June 2026; move without lease/POA | $750/wk cycles; mixed roles = liability | context-core §5; operating |
| Co-occupant (mixed) | Dan | Move-in with Suz; BFS parallel | Exposure via status | context-core, BFS bootloader |
| Contractor (paint/mech) | [[wiki/people/arnu]] | 2026-02-10 no-show noted; lien ~07-27 | Mechanics lien risk | message-csv 2026-02-10; timeline |
| Contractor (general, unreachable) | [[wiki/people/john-carney]] | ~10-20% complete | Incomplete work exposure | timeline, risks; Suz "John carney thing" 2026-03-27 |
| Worker/painter (assoc. Arnu) | Felix | 2026-02-10 showed; "paint more"; Elijah homework | Supply-adjacent per MAX_PRIME | message-csv; arnu page |
| Oversight | Jackson Alexander | Title | Liens attach | jackson page |

## Status
`status: active`

**Key Data from sources:**
- 337 sale contingencies active.
- 463: No lease/POA at move.
- Contractor activity: paint (Arnu/Felix) 2026-02; John Carney analysis shared 2026-03.

## Timeline Highlights
- 337 Saratoga sale in progress (contingencies).
- Move to 463 Morgantown (caretaker role).
- 2026-02-10: Arnu/Felix at property (Suz: "Felix showed up but Arnu didn’t... paint more").
- ~2026-03-27: John Carney work/"thing" discussed.
- 2026-06: 337 sale; no post-close plan.
- ~2026-07-27: Arnu lien deadline.
- Ongoing: Contractors, Suz real estate.

## Notes
Cross to people/suz, jackson-alexander, arnu, john-carney, felix (new from csv). From core: 337 being sold June 2026, no post-close plan. Parallels BFS authority/low-trust (vertical-authority-skepticism, work-power-dynamics).

**Sources:** raw/self/dox-md/operating_manual.md, context-core, BFS_BOOTLOADER, dox-scan, message-csv (Arnu/Felix/John Carney refs).

- [[wiki/legal/463-morgantown/timeline]]
- [[wiki/legal/463-morgantown/risks]]
