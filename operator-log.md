...
_Append-only. Written by `bin/wiki-gaps`; do not hand-edit — the tool marks
rows integrated when `bin/wiki-gaps clear <page>` runs._

**Read this at the start of a session.** Every row with an empty
**Integrated** cell is context the operator has supplied that no pass has
acted on yet. That outranks queued work: the evidence is already in the
repo and the only thing missing is the pass that applies it. The protocol
for applying one is CLAUDE.md's CLOSE operation — treat the answer as T0
testimony rather than proof, integrate rather than append, cascade into
every page that inherited the gap, then clear.
...
| 2026-08-18 | wiki/people/diane-shrum | staged-elsewhere | The gap, as this page stated it: | raw/people/captures/2026-08-17_230820_gap-diane-shrum.md | |
