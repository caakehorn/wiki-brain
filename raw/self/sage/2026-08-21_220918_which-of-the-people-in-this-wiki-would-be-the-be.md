...
revision: 3 — supersedes revisions 1 and 2, both published the same day
targets:
...
the same day and revised twice the same day, which is itself part of the record:
...
- **Revision 2** replaced the reasoning with quoted, dated evidence and retracted
  ...
  existed anywhere in `wiki/` or `raw/`*. That was also false. The verification
  behind it was a grep for `ENFP|INTJ|INFJ|ISFJ`, the four types the fabrication
  named; `INTP` was never searched for, and `wiki/mind/profile/intp.md` is a full
  page with a measured function table.
- **Revision 3**, held here, carries both corrections in the open and uses the
  INTP material where it actually bears — the 10%-valued Fe and its documented
...
refusal of Dan's redefinition move, which [[wiki/mind/profile/intp]] independently
names as the Fe-inferior mechanism; the Witness function performed and survived;
...
**The methodological finding, which outlives the question.** Revision 2's error is
the more instructive of the two: a check shaped by the claim it is testing will
confirm whatever that claim assumes. Searching for the four types a fabrication
happened to name, and reporting their absence as the absence of all, is a
verification that cannot fail to agree with itself.
