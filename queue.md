...
## Google Drive — DANMODEL folder (spotted 2026-07-20, NOT yet mined)
- [ ] HIGH candidate: Drive folder "DANMODEL" (under **DOX/~~DOCS) contains an unmined ML project, distinct from anything currently on wiki/work/tech: `reaction_extractor.py`, `rag_simulator.py`, `retriever.py`, `eval_harness.py`, `reaction_model.py`, plus two training-data exports `reaction_pairs_train.jsonl` (16MB) and `reaction_pairs_heldout.jsonl` (2MB) — reads as a self-modeling pipeline (extracting Dan's own message "reactions" into training pairs, then training/evaluating a model against them). Worth checking against wiki/work/tech/max-framework/overview.md and wiki/mind/concepts/exocortex.md before writing — may be a new project or an earlier/parallel iteration of the same exocortex work. Also in that folder: `PHENOMENOLOGY_LENS.md` and `CONTEXT_CORE_EXPANDED (1).md` — almost certainly duplicates of already-ingested raw/self/context-core sources (check filenames/hashes before re-filing); `extraction_summary` (1KB doc, unread).
