...
5. **Dates are absolute, and a hedged date is a question, not a resting state.** Convert relative time at write time; flag uncertainty as `(~2019?)` — then run `bin/wiki-corroborate window` on it, because 282,050 dated messages sit in `raw/` and the corpus can usually settle it. A hedge inside the record's coverage (2015-11 to 2026-08, with real holes inside it — 2012–2014, 2022, August 2017, everything before 2015-11-28) is a page telling you where to point the instrument, and `bin/wiki-corroborate scan --hedged` is the list of them.
