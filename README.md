...
- **Public mirror** (resynced hourly, useful when Pages is mid-deploy):
  https://caakehorn.github.io/leviathan/data/wiki-data.json — full page text
...
Six files govern the work, in the order a new reader should meet them. Each
says what it wins on when two disagree.
...
Plus the working files: `BACKLOG.md` (standing work), `LLM_HANDOFF.md` (the
exact resume point), `index.md` (master navigation), `log.md` (history of every
operation), `queue.md` (pending ingestion), and the two machine-maintained
queues `connection-queue.md` and `synthesis-queue.md`. `INGEST_RUNBOOK.md` and
`INGEST_PROTOCOL.md` cover the two ingest routes;
`FACTSTORY_BRIEF_TEMPLATE.md` is the source of truth for the brief shipped with
hand-typed captures; `AGENT_ACCESS.md` covers online access for agents.

`DIGEST.md`, `RECENT.md` and `OPEN.md` are generated by `bin/wiki-digest` —
