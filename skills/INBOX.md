...
- **Validation:** was `tests/test_wiki_skills.py::TestMoratorium::test_the_gate_catches_a_hand_edit_that_names_her`, removed on 2026-09-06 with the directive it enforced. **The candidate now has no validation and that is the honest state of it** — the invariant is untouched and needs a fresh pin on a generator that still gates its own output (`bin/wiki-plain report`'s hand-edit check is the nearest live one). Generalises `repo/change-safety.md` ("validate behavior, not merely successful commands") from HTTP 200 to a second case; promote after it catches a second instance in another generator.
- **Status:** provisional
