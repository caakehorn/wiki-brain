...
- **Validation:** reproducible — `bin/mine-tweets timeline '<narrow>'` and `bin/mine-tweets timeline '<broad>'` over the same archive disagree about 2017, 2022 and 2024. Promote after this catches a second instance; the named candidate is any future "the corpus never mentions X" claim drawn from `bin/mine-messages`, of which `wiki/interests/music/overview` already carries several.
- **Status:** inbox
