...
- **`lexicon/`** — words put to the wiki **from outside it**, the same shape as `sage/` and for the same reason. The portal has a word box; a word, piece of slang or phrase typed into it lands in `lexicon/words/` as a file. Nothing analyses it automatically. It is parked, `bin/wiki-work` lists it, and a session checks it **against the message record** before concluding anything, then folds the reading into `wiki/interests/language/vocabulary-lexicon.md`. Not in `raw/` because these files mutate (`pending` → `analyzed` | `rejected`). See `lexicon/README.md` and the LEXICON operation below.
...
### LEXICON — analyse a word somebody caught

A word noticed in passing has a short half-life. By the time there is a session
with time to work out what it means and whether it is his, the word is gone.
`lexicon/words/` catches it at the moment somebody has it; this operation is
what drains the queue. Full format and protocol in `lexicon/README.md`.

The one step that must not be skipped is **step 2: count it before deciding what
it is.** `bin/mine-messages grep` and `bin/text-metrics` answer the only
question that separates a finding from an impression — *does he actually use
this, and since when?* A word he likes and never says is a different fact from a
word he says four hundred times, and
`wiki/interests/language/vocabulary-lexicon.md` carries exactly that
distinction as its central caveat: it records words **selected as pleasing**,
not words **observed**. A checked word is the first kind of entry that page can
hold without the caveat.

The queue's first entry demonstrated why the step is mandatory. `choom` was
taken off `the-cool-metric`'s "documented persona slang" list to test the loop;
it appears **once** in 106,629 sent messages, and the six-term list it came from
totals nineteen — a list being used as *observed* evidence that turned out to be
the same kind of artifact as the curated one it was set against. One of its
terms was outright misfiled. None of that is visible without counting.

A word that turns out to be nothing gets `status: rejected` and a line saying
so. **Kept, never deleted** — a word checked and found empty and a word nobody
has looked at yet must not look the same from outside, which is the rule `sage/`
applies to a declined question.
...
| `bin/wiki-work` (`word`) | parked words from the portal's word box surface here as obligations at priority 3 — below staged answers, since nobody is watching for a word to be answered |
