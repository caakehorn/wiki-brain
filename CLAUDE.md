...
- **`data/intake/`** — the **intake ledger**: an append-only event log of substances, supplements and consumables tracked as discrete units, written from the portal's `/ledger` and read by `bin/wiki-intake`. Not in `raw/`, for two reasons. It is not a source that arrived from somewhere else — it is a *primary instrument*, a measurement taken at the moment the thing happened, in a format designed before the first reading rather than reverse-engineered after it; and it is mutable, since new events append to the current month's shard forever, so filing it under an immutable archive would misdescribe the directory. It is the only dataset here that is measured rather than recalled: `wiki/health/cocaine.md` states a dosage arc reconstructed across twenty years from messages and memory, and this is the instrument that measures the same thing instead. A page citing it makes a much stronger claim than one citing a remembered range — and, per `SYNTHESIS_SPEC.md`, a pattern in this log is still a fact about this log until it has been read against the person. `data/intake/README.md` carries the event shape, the three measurement classes and the five refusals any reader of it must hold.
...
| `bin/wiki-intake` | **the intake ledger** — reads `data/intake/`, the append-only event log of substances and consumables tracked as discrete units, written by `/ledger` in the portal. `check` (the gate: the log parses, every reference resolves, no unit is overdrawn), `units`, `report <unit>`, `stats`, `timeline --from --to`. Shares no code with the writer on purpose: two independent folds of the same log agreeing is the only thing that actually tests the contract. Holds the refusals — a remainder is a bound while any event was logged without a figure, a mean dose is over quantified events and carries its denominator, closing never assumes consumption. `data/intake/README.md` |
...
bin/wiki-intake check                                              # the intake ledger reconciles
