...
- **`intake/`** — the **intake ledger**: a finite quantity enters the record, every known disposition of it is recorded after that, and at depletion the ledger reconciles what is known and preserves what is unknown. `events.jsonl` is an append-only log and the source of truth; `units.json` is a projection regenerated from it and safe to delete; `substances.json` is the select box. Its point is not dose-counting — it is the anti-unreliable-narrator layer, a first-party dated dataset to set against a page's narrative the way `bin/text-metrics` is set against a page's claims about how he writes. **Its data is gitignored** (`intake/events.jsonl`, `intake/units.json`, `raw/health/intake/`) because this repository is public; the tool is tracked, the record is not. Those three lines were removed on 2026-08-30 on the stated ground that the repo had been made private — it had not, verified anonymously against the GitHub API the same day — and were restored by operator decision. **To reverse it: make the repo private first, verify it, then delete the lines, in that order.** Note what `.gitignore` does and does not cover: it governs `git add`, so it guards the CLI and the local app, and has no effect at all on GitHub's contents API, which is how the portal writes from a browser. That path has its own guard — **the portal reads this repository's visibility and refuses to sync while it is public.** Two guards, two paths, neither a substitute for the other. Three interfaces over one code path: `bin/intake`, Special:Intake in the local app, and `/ledger` in the portal — which is the one that is in his pocket, and therefore the one most of the record will arrive through. A finding drawn from the ledger reaches a page through the normal operations, cited to a unit id and a date range **with its coverage figure attached** — never as a bare mean. Three interfaces over one code path: `bin/intake`, Special:Intake in the local app, and `/ledger` in the portal — which is the one that is in his pocket, and therefore the one most of the record will arrive through. `intake/README.md`.
