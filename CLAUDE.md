...
- **The portal** — [`caakehorn/home`](https://github.com/caakehorn/home) renders this wiki, and its `public/wiki/**` is a **derived snapshot of `wiki/`, not a second copy of it.** A workflow there re-runs the derivation against this repo on dispatch *and every five minutes*, deleting the directory and rebuilding it, so **anything written into `public/wiki/` is destroyed within minutes** — including a change that merged. If a session finds itself editing a page as JSON, it is in the wrong repository: pages are `wiki/**.md`, here. This is not a style preference; two December 2015 read passes were written into the snapshot and one was reverted 39 minutes after merging (restored 2026-08-17). **How long that takes depends on who made the edit, and the difference is worth knowing.** An edit made *from the portal* — a page saved, a question asked, a word caught — has the site's own browser code behind it, which fires `wiki-updated` at the portal directly and lands in seconds. An edit merged *here*, as a pull request, has no browser and fires nothing: it waits for the portal's schedule, so **give it five minutes, and up to fifteen when GitHub is busy.** That is not a fault to go looking for, and there is no secret to check.

  `.github/workflows/notify-portal.yml` was this repository's half of the fast path and is **dormant since 2026-08-31** — `workflow_dispatch` only, `push:` commented out. It needed `PORTAL_DISPATCH_TOKEN`, which went missing on 2026-08-28 while the workflow went on reporting success and sending nothing for three days; the portal's schedule was moved from hourly to five-minutely to cover the gap instead, and no cross-repository credential is now required by anything. The file explains how to bring it back if the wait ever stops being worth it. The four paths the portal's `sync-wiki.mjs` actually reads are `wiki/**`, `sage/questions/**`, `plain/**` and `lexicon/words/**`.
