# Wiki Rewrite — Task Tracker

## Phase 0: Architecture
- [x] Style guide critique completed
- [x] Execution order finalized
- [x] Files confirmed extracted (268 files already in wiki/)

## Phase 1: Core Spine (highest priority)
- [x] `wiki/self/context-core.md` — frontmatter upgrade + LLM brief
- [x] `wiki/self/overview.md` — frontmatter upgrade + LLM brief
- [x] `wiki/self/index.md` — rewrite with LLM quick brief

## Phase 2: Mind / Concepts
- [x] `wiki/mind/concepts/abyssal-architect.md` — prose rewrite
- [x] `wiki/mind/concepts/attachment-model.md` — prose rewrite
- [x] `wiki/mind/concepts/conflict-architecture.md` — prose rewrite
- [x] `wiki/mind/concepts/dans-law.md` — prose rewrite
- [x] `wiki/mind/concepts/forensic-analysis.md` — prose rewrite
- [x] `wiki/mind/concepts/phenomenology-lens.md` — prose rewrite
- [x] `wiki/mind/concepts/contact-gini.md` — prose rewrite
- [x] `wiki/mind/concepts/cato.md` — prose rewrite

## Phase 3: Mind / Synthesis
- [x] `wiki/mind/synthesis/forensic-methodology.md` — prose rewrite
- [x] `wiki/mind/synthesis/attachment-trauma-bond.md` — prose rewrite
- [x] `wiki/mind/synthesis/totality-themes.md` — prose rewrite
- [ ] `wiki/mind/synthesis/lossless-retention.md` — prose rewrite
- [ ] `wiki/mind/synthesis/millennial-digital-witness.md` — prose rewrite
- [ ] `wiki/mind/synthesis/forensic-self-construction.md` — prose rewrite
- [ ] `wiki/mind/synthesis/aesthetic-politics.md` — prose rewrite
- [ ] `wiki/mind/synthesis/family-heritage-and-roots.md` — prose rewrite
- [ ] `wiki/mind/synthesis/vertical-authority-skepticism.md` — prose rewrite
- [ ] `wiki/mind/synthesis/work-power-dynamics.md` — prose rewrite

## Phase 4: People (primary cast)
- [x] `wiki/people/annie.md` — frontmatter upgrade + prose polish
- [ ] `wiki/people/fran-whyel.md` — full prose rewrite
- [ ] `wiki/people/suz.md` — full prose rewrite
- [ ] `wiki/people/rick-frank.md` — full prose rewrite
- [ ] `wiki/people/anita.md` — full prose rewrite
- [ ] `wiki/people/brandon-hill.md` — prose polish
- [ ] `wiki/people/jackson-alexander.md` — prose rewrite
- [ ] `wiki/people/eli.md` — prose rewrite
- [ ] `wiki/people/tom.md` — prose rewrite
- [ ] `wiki/people/morley-frank.md` — prose rewrite

## Phase 5: Timeline
- [ ] `wiki/timeline/periods/2017-poverty-floor.md` — full prose rewrite
- [ ] `wiki/timeline/periods/2018-deep-cycle.md` — full prose rewrite
- [ ] `wiki/timeline/periods/2021-2023-employment-block.md` — full prose rewrite
- [ ] `wiki/timeline/periods/2025-collapse.md` — full prose rewrite
- [ ] `wiki/timeline/periods/dec-2025-spike.md` — full prose rewrite
- [ ] `wiki/timeline/events/eli-incident.md` — full prose rewrite
- [ ] `wiki/timeline/events/group-chat-closure.md` — full prose rewrite
- [ ] `wiki/timeline/events/drawer-dispute.md` — prose rewrite

## Phase 6: Interests
- [ ] `wiki/interests/music/overview.md` — prose upgrade
- [ ] `wiki/interests/music/aliases/gripnotic.md` — prose upgrade
- [ ] `wiki/interests/music/concepts/sub-bass-signature.md` — prose upgrade
- [ ] `wiki/interests/favorites/books.md` — prose upgrade
- [ ] Author stubs (5) — full rewrites from 3-line stubs to real articles
- [ ] Music artist stubs (10) — full rewrites

## Phase 7: Legal
- [ ] `wiki/legal/bfs-dispute/overview.md` — rewrite with status banner
- [ ] `wiki/legal/463-morgantown/overview.md` — rewrite with status banner

## Phase 8: Indexes + Style Guide Files
- [ ] `wiki/self/index.md`
- [ ] `wiki/people/index.md`
- [ ] `wiki/mind/index.md`
- [ ] `wiki/interests/index.md`
- [ ] Write final `STYLE_GUIDE.md` into wiki root
