## [2026-08-13] audit | self+mind+people | a 186k-row whole-device export audited against the wiki — one attribution inverted, two coverage artifacts, one operator decode

- **The corpus rendered an authored act as a befallen one, twice.** (1) `sic semper lupanis`, the relationship's closing line, was attributed to Annie on five pages. It is Dan's: `2026-06-01 00:27:49 America/New_York | Sent | HANDLE=Me | dyad +12124702449`, the last of an unbroken outbound run beginning 00:21:47, with the next inbound row four days later. Cause is in the wiki's own citation string — `Sent (Received by Dan)`, a direction column written from Dan's POV and read as Annie's outbound. `group-chat-closure.md` held the contradiction *inside one table cell*, labelling the line "(Annie)" while listing the message 110 seconds earlier as Dan's. (2) The February 2025 eviction from 307 E 76th, recorded as a landlord losing patience, was arranged by Dan — he phoned John Paci and asked him to perform a filing so Annie would believe the exit was forced. Sole source is operator testimony (`raw/self/captures/2026-08-13_john-paci-staged-eviction-operator-decode.md`); the corpus cannot produce it, because the misleading evidence was constructed deliberately and messages were deleted at the time. Corroboration: a 167-day hole in the Paci thread covering the setup, and one surviving line — `"Sounds good.  Thanks for the help"` — thanking a landlord for help 28 minutes after a message that answers an outbound row no longer in the record.
- **The self-closure thesis moved and was worked rather than deferred.** `dormancy-not-exit`, `block-unblock-loop` and `the-unbroken-bond` shared a claim that Dan cannot close a bond from the inside without an external severance signal. He wrote the closure himself and held it 52 days against four unanswered approaches. Revised: the architecture implements **suspend**, not **delete**. The "she issued zero severance signals" half gets *firmer* — the one line read as her termination was his, so Annie has no exit line in the record at all.
- **Two coverage artifacts had become claims.** The 2021-04-27 → 2023-08-09 hole (~833 days, broken only by six outbound rows on 2022-12-31, two of them literally "test") is an export failure, not silence: the location record logs ~690 work shifts and ~1,791 place visits inside it. `message-circadian-latency.md` called it "the corpus's hardest floor"; `2021-2023-employment-block.md` called it "not a gap in the record but the shape of the era itself"; `rick-frank.md:313` reasoned from it. All three now disclose it. A second, non-aligned artifact: no SMS rows of any kind 2020-07-30 → 2024-05-24, boundaries replicating in the independent dump, consistent with Text Message Forwarding disabled.
- **A synthesis page's headline reversed on recomputation.** `message-circadian-latency.md`'s 9× reply-latency asymmetry does not reproduce on any of three sources under any of three readings of its own stated method — Annie is *faster* than Dan (0.62 vs 0.75 min median). Two diagnosed causes: the page's two latency columns are transposed (the forward reading reproduces its inbound column as Dan's own latency), and its declared source drops ~8,500 of Dan's outbound on one 2025 thread, inflating every inbound gap. The page also contradicts itself at line 57 before any file is opened. RETRACTED. The circadian curve (22:00 peak 7.5%, night share 15.5%) reproduced exactly and survives.
- **What held under attack, recorded because it counts.** The verbal-abuse asymmetry — 74 / 17 / 11 against 0 / 1 / 0 — was attacked twice and reproduces exactly (word-boundary per-occurrence = 74). "The asymmetry is complete" is correct. Also surviving adversarial re-derivation: the 52-day block, `dormancy-not-exit`'s prediction #1, and the 2026-08-11 Rick correction, corroborated from a source it never used.
- **Identity.** `+13307038747` = Kristin Prentiss — 9,907 rows, absent from `contacts.csv`, which is why every automated pass failed to connect the handle to a page that already existed. `+12532025869` = Trevor. `johnny-dealer.md` and `john-anderson.md` are one man across a two-step number migration whose third handle appears nowhere in the repo.
- **Source ranking, now recorded.** `all_imessages_complete_dump.txt` (2025-08-10) beats the 2026-08 deep export for 2019–2025: 41.8% of the older file's records have no row in the newer one. The deep export's `DATE/TIME` is UTC against the wiki's America/New_York — validated on 42,895 uniquely text-matched pairs (23,158 at +5.00h, 19,692 at +4.00h). Both exports are now filed with a provenance README.
- **Process failure, logged honestly.** PR #109 merged carrying **1 of ~25 intended files** because the run producing it died on a quota limit, while its description claimed the full pass. Nothing checked, and no `log.md` or `LLM_HANDOFF.md` entry was written, so the work was invisible to the next session by construction. Three subsequent orchestration attempts died the same way. The corrections in this entry were ultimately written serially, committing after each file.
- Pages touched: `annie-ulmer`, `dan-annie-fallout-verdict`, `dormancy-not-exit`, `block-unblock-loop`, `the-unbroken-bond`, `john-paci`, `307-e-76th-st`, `feb-apr-2025-return-and-rupture`, `message-circadian-latency`, `2021-2023-employment-block`, `rick-frank`, `master-message-dump`, `group-chat-closure`, `end-fight`, `tuquick-17248123683`, `node-locking`. New raw: the operator capture and both 2026-08-13 exports.
