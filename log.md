...
## [2026-08-08] tool | meta | wiki-monolith — whole corpus as one document, size-optimised
Operator asked for everything in the wiki as a single document, optimised for smallest filesize. Added `bin/wiki-monolith`, which concatenates all 438 pages with frontmatter intact and emits the document plain plus one file per available codec, verifying every artifact round-trips byte-for-byte against the pages on disk.

The measurement (reproducible with `--all`) settles which levers are real. Compression is the only large one: 2.55 MB plain → 521 KB PPMd order-16, a 5.0:1 lossless win, with brotli (638 KB) and text-tuned xz (644 KB) close together well behind it and gzip (892 KB) far back. The two levers that look promising are not. Substituting the 442 repeated `wiki/` paths for symbols saves 164 KB of plain text (6%) but only 6 KB of xz and nothing at all of PPMd — a compressor already models that redundancy better than a symbol table does — so `--compact` is off by default and touches only the plain file. Stripping frontmatter is worse than it looks: it is 23% of raw bytes but only 11% of compressed size, and the `connections:` claims inside it are earned content, so the monolith keeps everything.

Practical upshot: ship `.txt.xz` when the reader must be able to open it with standard tools, `.txt.ppmd` when 19% matters more than universal decodability. Two bugs found and fixed during verification — variable-width symbol ids were prefixes of one another and corrupted expansion (now fixed-width hex), and the delimiter is now guarded against occurring in page text rather than silently splitting a page. No wiki pages were touched; all three gates at 0 errors.

## [2026-08-08] tool | meta | wiki-monolith gains txt/csv plain-file output
Operator clarified the target: an uncompressed .txt or .csv, not an archive. That inverts the earlier finding. Compression was the only lever that mattered when compression was allowed; with it off the table, the small text-level levers are all there is, and the ceiling is low.

Added `--format csv` (and `--plain` for a bare .txt). CSV is the smallest clean plain file at **2.49 MB** versus 2.55 MB for concatenated text, because 438 repeated YAML key names collapse into a single header row (−48 KB) while quote-doubling costs only 9.8 KB. Stripping the redundant `wiki/` prefix from every path saves a further 30 KB. A path symbol table gets to 2.40 MB but litters the file with control characters, so it stays opt-in.

The honest ceiling: everything kept, lossless, you land at ~2.5 MB in any plain format. It is 2.5 MB of actual prose and there is no trick left. The one large cut available is dropping frontmatter — 1.97 MB — and it is refused by default, because that is where the typed `connections:` edges and their argued claims live.

CSV is a restructuring rather than a concatenation, so it cannot be byte-verified the way the .txt is; `--verify` instead checks every row present, every body exact, and every frontmatter value recoverable. All 438 rows pass. Output is 19 columns: path, the promoted scalar frontmatter fields, `sources`/`connections`/`synthesizes`/`related` as verbatim blobs, a catch-all `other_frontmatter` so no field is ever silently dropped, and `body`.
