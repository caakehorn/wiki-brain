...
## [2026-08-08] ingest | people | the interloper is named — FOREWARN capture into tuquick-17248123683
Operator capture (`targets: wiki/people/tuquick`), filed to `raw/self/captures/2026-08-08_190122_identity-of-the-interloper.md`.

**The identification is strong because the corpus supplied half of it.** FOREWARN returned **Jerel Wayne Coles, 36, Uniontown PA**, and lists a mobile — `(724) 812-3683` — that is digit-for-digit the handle this page has been built on since June 2026. The wiki had the number first and the name second; they meet on a record match rather than on a name search. Recorded with the source tier stated: FOREWARN is a commercial aggregator sold to realtors, not a court system, and while the phone-to-name link is a direct match, the name-to-criminal-record links are probabilistic — the capture preserves FOREWARN's own "Match Key Criteria Met" admission (first name, last name, middle initial, birth date, city, state, proximity). Nothing is verified against a Fayette County docket and the page says so.

**The 64 records are about nine incidents, and the capture's own deduplication is the valuable part.** Rows repeat across the PA Administrative Office of Courts, the Unified Judicial System and the Court of Common Pleas, and across procedural stages of one case. Collapsed by offence date: **~9 clusters, ~20 distinct charges** — traffic matters and two minor public-order offences 2008–2010, a **first DUI 2019-02-03 at BAC .16+ resolved through ARD**, a **second DUI 2024-06-29 with guilty pleas on all five counts**, and a **2025-04-05 criminal mischief** with no disposition. Tabled on the page. Describing this as "64 offences" would have been wildly misleading and the capture says so itself.

**The load-bearing rows are the two smallest.** A **2008 disorderly-conduct/fighting guilty plea** and a **2009 harassment — physical contact conviction** are the only evidence from outside Dan's or Annie's account that the 2026 conduct belongs to a pattern. Kept deliberately narrow: two summary offences from a man's early twenties corroborate a *class* of behaviour, not any specific allegation, and explicitly **not** the July 2026 allegations on `the-unnamed-man`, which concern a person not established to be the same man. What they do bear on is the one incident the corpus witnessed directly — the three-hour phone harassment of Suz in May 2026, which until now sat in the record with no context at all.

**The open question moved sideways, not closed.** Coles is placed in Uniontown from September 2024. `the-unnamed-man` is placed in Uniontown by Annie's account. That **removes a way of closing the question negatively** — "Tuquick lives elsewhere" is no longer available — without establishing identity. Both pages now say this in the same words; neither claims more.

**Deliberately not recorded: the street address and the residential number.** `STYLE_GUIDE.md` already requires masked phone numbers, the full prose of every page is republished unauthenticated to the public leviathan mirror, and neither datum does analytical work the county and the name do not already do. Both remain in the capture in `raw/`.

**Staleness:** `dan-annie-fallout-verdict` went stale and was re-read, not bumped. The June 15 defection is untouched — a witness's record does not change whether he said the words — but the re-check tightened the page's use of "independent": the defection's value was always that Tuquick was *hostile* and moved anyway, not that he is reliable, and the page now warns against using the new criminal record as second-hand corroboration of Annie's account. Gates: wiki-lint **438 pages / 0 errors** (13 warnings) · wiki-connect check **0 errors** · wiki-climb check **0 errors, 0 warnings**.

**Not done, and flagged rather than dropped:** the `/wiki-rewrite` pass on `tarik-fallous` + `valeria-iglesias-cid` + `au-zaatar` was interrupted mid-research with nothing written. Findings already established and unrecorded: the GPS record holds **447** Au Za'atar visits (444 Midtown at 1063 1st Ave, 3 East Village at 188 Avenue A), not the **445** three pages assert, and its span (2021-07-20 → 2024-03-31) covers neither end of the tenure, so the figure is a floor and its 121/64/256/6 per-year shape is Google's logging rate rather than attendance; the **"~690 shifts"** on `2021-2023-employment-block` is unreproducible from the location record at any grouping (433 distinct visit-days). `raw/self/dox-scan/Resume.txt` closes that page's named title gap — **"Waiter (Captain), March 2021 – August 2024"**, promoted to head waiter, with new-hire training and menu clinics — and independently corroborates dates the wiki took from AI-secondary storytime. Tarik's Aug 11 2023 all-zero wine list is **Received**, i.e. Tarik sending Dan a blank form which Dan returns filled the next day, not the "restock" the page reads it as. Dan opens **27 of 29** conversation-days with Tarik and the thread has an **18½-month silence** (2024-09-28 → 2026-04-12) the page's "still in contact as recently as April 2026" framing hides. Valeria's counts all verify, but the **"52% between 22:00 and 01:00"** figure is a 22:00–01:59 window (22:00–00:59 is 36.1%) and, against the only available baseline — her own messages, from a timezone with the same offset — **37.4%**, so the night-class-alibi inference does not survive a control. Four contemporaneous June 2023 ChatGPT sessions (`Au Za'atar NYC Description`, `Unknown Person: Tarik Fallous`, `Lebanese Cuisine Overview`, `Shish Taouk Overview`) and an Aug 1 2022 Tom exchange about Valeria are in none of the three pages' `sources:`.
