...
## [2026-08-10] ingest | mind, people, work | Facebook deep-mine — the Tom Maison thread alone rewrote four pages
Operator: "continue to scrape and analyze potential untapped gold mines of data. if there's nothing particularly substantive that is available then we need to do a much more thorough and deep scan of my facebook messages and general facebook account history/activity." Surveyed `queue.md`, then audited the existing `wiki/self/facebook/*.md` sub-pages against the raw export and found the gold mine on the first try: those four pages (messages, posts, friends, interests) were written as category-level overviews — "403 threads... real signal concentrated in key nodes" — not read to exhaustion. Only a handful of the 271 inbox threads had ever actually been opened.

**Read one thread in full: `messages/inbox/tommaison_z_scvtnccq/message_1.html`.** At 1.7MB and 5,439 messages (May 2008 – Aug 2022) it's the single largest thread in the export — larger than every other thread combined by a wide margin — and belongs to Tom Maison, already one of the most-developed pages in the wiki, with **zero Facebook sourcing on it before this pass.** One thread, five findings, four pages touched:

1. **`tom.md`'s friendship-origin date was wrong, and traceable to trusting a filename.** The page said "roughly 2010," sourced to `dan tom 2010 2022.txt` — a raw filename, not a dated primary claim. The actual export: earliest message May 3, 2008; a Facebook system notification on Dec 24, 2019 reading "You and Tom Maison are celebrating 14 years of friendship on Facebook," placing the real friend-connection at ~December 2005. CORRECTED block added; the friendship is at minimum 17 years old at the point the corpus ends, not "roughly 2010."
2. **A previously undocumented Jan–Feb 2021 crisis cluster for Tom, not Dan.** In roughly three weeks: his dog died (Jan 18), a girlfriend left him after a NYC visit where — by his own account — "we got a hooker," a DUI was "about to kick my ass" (a second, earlier DUI distinct from the already-documented fall-2025 one), a single unelaborated reference to a sister's death implied by suicide ("taking everything I have to not go out like my sister did," Jan 20 — recorded with care, not embellished beyond what he actually wrote, since it's the only mention in the corpus), and a rehab stay by mid-February. New section on `tom.md`.
3. **Jim Shaffer's 2017 embezzlement arrest gets independent corroboration.** `new-jim-shaffer.md` previously rested entirely on Jim's own 2018 self-serving account ("terrible prelim investigation," a "smear campaign"). The Tom thread has Dan pasting the actual contemporaneous news article the night the arrest is charged (March 11, 2017): 37 felony counts, ~$190,000 across six investors, a mortgage guarantee secured against a house he didn't own, transactions dated 2014–2015 — a full year before the "left six months before it collapsed" framing Jim gives his own conduct in 2018. Both accounts are now on the page, disagreeing on scope, neither correcting the other.
4. **A second contemporaneous account of Rob Orange's death.** `rob-orange.md` said Lauryn Ashly's thread was "the corpus's only contemporaneous account." It wasn't — Dan told Tom the same day, 2:12 AM, with a more confident overdose read than the ambivalence he shows Lauryn in the same window. CORRECTED.
5. **The J6-hypothesis's documented origin moves from June 2025 back to January 7, 2021 — the actual week of the event.** `political-psyops.md`'s own "Chronology note" had already pushed the thesis's origin back from the January 2026 Gemini codification to a June 2025 ChatGPT session. The Tom thread pushes it back over four more years: "i've been obsessively pouring over the footage and it is SO obvious that the cops were aware of what was happening and let them in" — the specific procedural-complicity claim the "Inverted Containment" doctrine formalizes half a decade later, already fully formed two days after the event. What doesn't carry forward is the political valence: the 2021 Dan is sharply critical of the rioters and of Trump specifically, arguing a populist-left position ("if trump was able to... deliver the type of pro working class / pro union policy plancks that bernie has, he would 100% be reelected") with no sympathy for the rally's cause. REVISED block written up as a real finding: the procedural-suspicion mechanism precedes the political interpretation by years, not the reverse.

**Minor corroboration, not treated as a correction:** a May 24, 2021 message pins Dan's own Au Za'atar hire *announcement*, about two months after the page's established March 2021 start date — recorded as the more precisely dated announcement without overturning the deliberately-corrected established date.

**What was checked and found low-value, so a future pass doesn't repeat it:** `your_topics.html` (Facebook's own algorithmic ad-interest tags — Bears, Deer, Labrador Retrievers — not personal signal) and `other_activity/pokes.html` (a handful of names). `queue.md` updated with a full ranked list of what's left: ~265 more message threads (several large and clearly under-mined the same way Tom's was — Bobby Cole's 332K thread is the next-largest), `comments_and_reactions/` (6.0MB, never opened, real opinion signal), `apps_and_websites_off_of_facebook/` (6.9MB, never opened), and several smaller unchecked categories.

**Staleness cascade:** `food-and-diet.md` and `the-embedded-objective.md` both flagged stale against `au-zaatar.md`'s minor addition; both re-checked and closed — the 41-month tenure figure both depend on was untouched by the two-month hire-date discrepancy.

New connections: `tom.md` ↔ `political-psyops.md` (evidences/evidenced-by), `tom.md` ↔ `new-jim-shaffer.md` (evidences/evidenced-by), `tom.md` ↔ `au-zaatar.md` (evidences/evidenced-by), `tom.md` ↔ `rob-orange.md` (evidences/evidenced-by).

Gates: wiki-lint **450 pages / 0 errors** (13 warnings, unchanged) · wiki-connect check **0 errors** (216 warnings, unchanged — all four new edge pairs matched cleanly on the second attempt after an initial evidences/co-occurs mismatch caught by the gate) · wiki-climb check **450 pages, 23 with `synthesizes:`, 0 errors, 0 warnings** after cascade closed. `bin/wiki-digest` and `bin/llm-publish` rerun.
