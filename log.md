## [2026-08-09] climb | mind | creator-economy-connector — new synthesis, cluster #12
- First CLIMB of the session. Took `synthesis-queue.md` cluster #12 (score 11.50: `arrangement-history`, `kya-hansen`, `shelbie-breakiron`) — the top-scoring cluster I had fresh, deep knowledge of, having just rewritten `shelbie-breakiron.md` and retrofitted `kya-hansen.md` in the prior pass this session.
- **The governing rule:** wherever Dan appears inside the commercial sex-work/creator economy, he tends to occupy a position above simple consumption — operator, moderator, connector — even in the one documented case with zero personal stake in the outcome. `kya-hansen.md` is the load-bearing exception that makes this a rule rather than a redescription of `arrangement-history`: every other row on that table (Shelbie, Trinity, Kelly Johansson, Alexis) has Dan as a paying or directly participating party; Kya Hansen has no message thread, no payment, no romantic or sexual content anywhere in the corpus — the entire relationship is unpaid subreddit moderation, followed a year later by two separate, dated referrals of her content to other people (Shelbie, June 2019; Jerad Friedline, March 2020, this one embedded in a genuine OnlyFans-economics conversation). Annie's own `smashonista` camming operation extends the pattern to the supply side — the couple aren't just adjacent to this economy, they sell into it.
- Falsifiers stated: a hidden romantic/commercial tie to Kya Hansen (unrecoverable OnlyFans message history is the one place it could still surface) would collapse the exception; a single one-off favor rather than a repeated, dated pattern would overstate a coincidence. Prediction: future creator-economy instances should skew toward operator/promoter roles, some with no personal stake, or the rule is wrong.
- **Write-back:** `instantiates`/`instance-of` pairs added to all five member pages (`arrangement-history`, `kya-hansen`, `shelbie-breakiron`, `jenn-lynn`, `annie-ulmer`) plus `parallels` edges to `orchestration-and-voyeurism` (same "director/above the economy" shape, different mechanism) and `acquisition-drive` (same completion-drive read, applied to unpaid moderation instead of caregiving). Fixed a connections-type pairing on the way (my own first pass used `instantiates` on the synthesis page's outbound edges instead of the correct `instance-of`).
- Staleness: `acquisition-drive.md`'s edit moved `the-embedded-objective.md` past its premise. Re-read: the new Kya Hansen parallel is a self-chosen role, not an externally-assigned one, so it isn't a fifth test case for that page's assigned-vs-self-generated distinction — the falsification finding stands. RE-CHECKED block added.
- Also removed a bare `## Related` footer on `orchestration-and-voyeurism.md`, surfaced only because adding a `connections:` entry there made the lint check notice it.
- Gates: `wiki-lint` 440 pages / 0 errors (13 warnings) · `wiki-connect check` 0 errors / 192 warnings (down from 199) · `wiki-climb check` 440 pages, 22 with `synthesizes:`, 0 errors, 0 warnings. `bin/wiki-digest` and `bin/llm-publish` rerun.
- `synthesis-queue.md` cluster #12 marked `CLIMBED -> wiki/mind/synthesis/creator-economy-connector`.

## [2026-08-09] rewrite | people | shelbie-breakiron, jason-bermejo, eric-jester — wipe and re-research
- Operator directive: full wipe-and-rewrite of three person pages via the `wiki-rewrite` skill (its first real use since being written). All three had passed gates and looked complete; the skill's premise held anyway — none had been researched to exhaustion, and the widest-reaching finding (Eric Jester's Fran vigil account) was sitting in a source none of the three pages' `sources:` lists had ever opened.
- **Jason Bermejo — a fabricated source and a fabricated fact, corrected.** The prior page cited `imessage_export_+18172693422_20260714055318.csv` for a "Feb 2026 renewal, 400+ messages in a month, all-time peak." **That file does not exist anywhere in `raw/`.** `all_imessages_complete_dump.txt` is complete for this handle through the thread's real final message (2025-03-17) and shows zero 2026 activity. Removed the fabricated material with a `CORRECTED` block rather than silently dropping it; real corpus-supported figures (292 messages, 2017–2025) replace the inflated ones. Also rewrote the entire body from bullet-fragment "dossier shorthand" into STYLE_GUIDE prose — the whole page below the intro paragraph had never actually been converted despite a 2026-07-19 pass touching the frontmatter.
- **Eric Jester — the page's real date range was off by eight years, and an entire source was never opened.** The prior page cited only `MASTER_MESSAGES_DB_DUMP.csv` (direction-broken by design) and reported "103 messages... direction unreliable... marked Received throughout." A 220-message Facebook Messenger archive (Jan 2009 – Sep 2020) sitting in `raw/self/facebook/` was never searched for. Real combined record: 2009–2025, 235 clean bidirectional iMessage/SMS messages (133/102) plus the Facebook archive. Inside that archive: Dan's own, highly detailed real-time account of the end of Fran Coldren's life (April 5–6, 2018) — paid $15/hour caregiving, a silent bedside "give me a sign" test, his girlfriend waking him the instant Fran blinked, the whole thing on camera, and the inheritance named as his reason for leaving PA the same week she died. This is a **second, independently-worded telling** of the same core act [[wiki/timeline/events/fran-death-vigil]] already carries from a same-night message to Lucie Dobbin — and the two do not match in every detail (blink vs. smile; falling asleep vs. staying awake; recovery vs. death itself). Recorded side by side on both pages rather than harmonized. Also found: Dan's Oct 2011 Occupy Wall Street participation (predating the wiki's "2020 left turn" framing by nine years), Eric's own workplace suspension (a thrown lemon, not a punch), a Boston Marathon near-miss ("Kendel was 2 blocks away"), and — the single best find — a July 5, 2013, 1:31 AM message ("dude i got punched in the face by alexis today hahaha") that is the first primary-dated evidence located anywhere for [[wiki/timeline/events/franki-fireworks-day-2013]], a page whose own Gaps section had called the date unfixed since 2026-08-02.
- **Shelbie Breakiron — smaller delta, real one.** The 685-message count held up against the per-thread CSV (the complete-dump's lower count, 471, is logged as an unexplained dump-coverage gap, not treated as more authoritative by default — STYLE_GUIDE's "table the evidence, say which governs" applied literally). A standalone RTF capture, `Shelbie Breakiron on 2019-04-17 at 16.08.35.rtf`, gives a dated single message from **April 17, 2019** — the earliest primary contact in the corpus, a full month before the CSV thread opens and twelve days before the April 2019 threesome's stated date. Also wired in two previously-unlinked cross-references already sitting in other pages' prose: Kya Hansen (Dan referred her OnlyFans to Shelbie mid-relationship) and Jenn Lynn (an explicit `parallels` the Jenn page already named but never linked back).
- **Write-back was the majority of the work.** All three pages had declared outbound `connections:` whose targets never got the inverse edge — the exact failure CLAUDE.md names as most common. Added real inverses, several with load-bearing prose, across: `chemical-architecture` (from the prior PR, unrelated), `annie-ulmer`, `shelbie-annie-threesome-april-2019` (retrofitted off `related:` + a bare `## Related` footer), `kya-hansen` (retrofitted), `jenn-lynn`, `arrangement-history`, `trinity-st-clair` (retrofitted off `related:` + a bare footer), `jerad-friedline`, `2018-deep-cycle`, `jamie-mohler` (fixed a mismatched `co-occurs`/`evidenced-by` pair to `evidences`), `franki-fireworks-day-2013`, `fran-death-vigil`.
- **Staleness cascade, not date-bumped.** Editing `fran-death-vigil.md` moved `suzanne-frank.md` (which `synthesizes:` from it) past its premise. Re-read: the new material is about Dan's own retelling and the inheritance motive, not about Suz's caretaking role, which is what that page's claim rests on. Conclusion survives; RE-CHECKED block added.
- Gates: `wiki-lint` 439 pages / 0 errors · `wiki-connect check` 0 errors / 193 warnings (down from 208) · `wiki-climb check` 439 pages, 21 with `synthesizes:`, 0 errors, 0 warnings. `bin/wiki-digest` and `bin/llm-publish` rerun.
- Pages touched (14): `wiki/people/shelbie-breakiron.md`, `wiki/people/jason-bermejo.md`, `wiki/people/eric-jester.md`, `wiki/people/annie-ulmer.md`, `wiki/timeline/events/shelbie-annie-threesome-april-2019.md`, `wiki/people/kya-hansen.md`, `wiki/people/jenn-lynn.md`, `wiki/mind/psychosexual/arrangement-history.md`, `wiki/people/trinity-st-clair.md`, `wiki/people/jerad-friedline.md`, `wiki/timeline/periods/2018-deep-cycle.md`, `wiki/timeline/events/franki-fireworks-day-2013.md`, `wiki/timeline/events/fran-death-vigil.md`, `wiki/people/suzanne-frank.md`; plus `wiki/people/index.md` summary lines.

## [2026-08-09] connect | timeline/health/people | robotussin's last dance — age correction and write-back completion (PR #91 fixes)
- PR #91 (external model, Hermes agent) ingested a manual capture, `raw/self/captures/2026-08-09_143916_robotussin-s-last-dance.md`, and wrote a new event page, `wiki/timeline/events/robotussin-s-last-dance.md`, on the June 15 2005 DXM/Tom Petty trip with Jim Shaffer and Rob Orange. The extraction and cross-referencing were solid — it correctly matched the capture against Jim Shaffer's Dec 2018 "suped up mini van" message already on two other pages — but it shipped an uncaught corpus-contradicting age error and left half its write-back edges one-directional.
- **Age corrected.** The capture self-reports "17-year-old boys" / "age 17" throughout; Dan's DOB is `1988-11-01` (`[DOC]`-tier, `CONTEXT_CORE_EXPANDED.md`), which makes him **16**, not 17, on June 15 2005 — five months short of his birthday. STYLE_GUIDE.md's capture-handling rule ("check every date, age and count against the corpus... table the evidence and say which governs") was not applied by the original pass. Corrected throughout `robotussin-s-last-dance.md` (prose, infobox `known_for`, the `rick-frank` connection claim) to 16/"teenage boys," with the discrepancy recorded explicitly in a new Gaps bullet rather than silently overwritten.
- **Missing write-back inverses added.** The new page declared `component-of` → `wiki/health/chemical-architecture` and `co-occurs` → `wiki/people/rick-frank`, but neither target page was touched by the original PR — `wiki-connect check` confirmed both as "no edge back" warnings. Added the inverse `contains` edge plus a load-bearing prose paragraph to `chemical-architecture.md` (the June 2005 DXM dose as the earliest instance of Dan naming a substance's purpose in the same register later used for the engineered stack), and the inverse `co-occurs` edge plus a prose paragraph to `rick-frank.md` (the same day's Frank's Auto Supermarket shift, with Rick already a "non-factor" by then). Also corrected a type mismatch on `teen-concert-years.md`: it carried `co-occurs` back to an edge the new page typed `component-of` (whose real inverse is `contains`) — retyped to `contains`.
- **Staleness cascade worked, not date-bumped.** Editing `rick-frank.md` moved its `date_modified` past four synthesis pages that `synthesizes:` from it — `block-unblock-loop.md`, `estate-money-spine.md`, `fayette-return.md`, `attachment-trauma-bond.md`. Re-read each against what actually changed (a same-day adolescent-drug anecdote, not the 12-day-burst pattern, the business's capital role, the no-attested-departure fact, or the 2005 rupture's substance); all four conclusions survive unchanged. RE-CHECKED blocks added to all four rather than bumping `date_modified` alone.
- Gates: `wiki-lint` 439 pages / 0 errors (13 warnings, all pre-existing) · `wiki-connect check` 0 errors / 208 warnings (down from 212 — the two missing-inverse warnings and the type-mismatch warning cleared) · `wiki-climb check` 439 pages, 21 with `synthesizes:`, 0 errors, 0 warnings. `bin/wiki-digest` and `bin/llm-publish` rerun.
- Pages touched: `wiki/timeline/events/robotussin-s-last-dance.md`, `wiki/health/chemical-architecture.md`, `wiki/people/rick-frank.md`, `wiki/timeline/events/teen-concert-years.md`, `wiki/mind/synthesis/block-unblock-loop.md`, `wiki/mind/synthesis/estate-money-spine.md`, `wiki/mind/synthesis/fayette-return.md`, `wiki/mind/synthesis/attachment-trauma-bond.md`.
