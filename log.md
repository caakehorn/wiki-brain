...
## [2026-08-02] verify | people | annie-ulmer re-measured against the raw exports — two headline figures corrected, one new asymmetry
Operator asked for the same complete reanalysis-and-rewrite treatment given to `fran-coldren`. **The reanalysis is done and written in; the from-scratch prose rebuild is not, and that is a deliberate stop rather than an omission** — see the note at the end.

Re-measured the page's load-bearing quantities directly with `bin/mine-messages`, patterns stated on the page so they can be re-run and disputed.

**Volume: the headline number was counting one side.** This page and `master-message-dump` both report 31,177 messages on Annie's early PA handle. That is her *inbound* only — an artifact of `MASTER_MESSAGES_DB_DUMP.csv`, whose Sent rows carry no `contact_handle`. From `all_imessages_complete_dump.txt`, where direction is reliable, the PA handle is **29,367 sent / 31,269 received = 60,636 two-way**, and the three Annie handles together are **108,819** in that dump alone (which excludes 2021, 2022 and 2026 entirely). The 31,269 received is almost exactly the 31,177 previously reported. The wiki's most-quoted relationship volume was roughly half the real one.

**"Fuck you" 74 was an extract, not a total.** The prose rendered a table row as "seventy-four times in seven months." Measured across `annie_all_time_logs.csv` (2022-12 → 2026-06-05) the count is **100**, peaking at **26 in February 2026** — one month before the confession-and-retraction. The 74 approximates the Aug 2025–Mar 2026 extract window; the count simply kept going after the extract stopped. Both the prose and the table row are corrected.

**An independent confirmation worth more than either measurement alone.** The existing monthly verbal-abuse curve (9 → 0 → 5 → 14 → 22 → 25 → 36, Aug 2025 → Feb 2026) came from a different pass with a broader keyword set. A "fuck you"-only count over the same months returns 6 → 0 → 2 → 10 → 11 → 11 → 26 — different scale, same shape, same February peak.

**The finding neither pass had: the exits invert.** The page's account is that Dan declared the relationship over 127 times and always returned, teaching Annie his exits carried no weight. True of the terminal phase, false of the decade before it. Exit language by direction: 2015–2025 dump — **Dan 103, Annie 113**; Dec 2022–Jun 2026 export — **Dan 62, Annie 4**. For most of the relationship Annie threatened to leave slightly *more* often than Dan; in the terminal phase she stops almost entirely. Exit threats look like a shared currency she withdrew from circulation at the point she had somewhere else to go. The same shape appears in suicidal statements, which the page has only ever counted for Dan: 2015–2025 **Dan 21, Annie 11**; 2022–2026 **Dan 18, Annie 1**.

**What was deliberately not done.** `fran-coldren` was 300 lines and a from-scratch rebuild improved it. `annie-ulmer` is 1,174 lines carrying a verdict held across four analytical passes, and a single-turn blank-page rebuild would have produced a shorter, worse page and silently dropped reasoning that cannot be re-derived — which is exactly the failure the Fran rewrite demonstrated in miniature (seven inverse edges and a contradiction block lost, caught only because a gate happened to look). The edge snapshot taken before this pass verifies **27/27 outbound edges intact, none lost**. The structural rewrite should be done section-by-section against that snapshot, not in one pass.
