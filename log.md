## [2026-08-11] ingest | legal/self | Ford trademark suit resolved, 23andMe health/ancestry percentages extracted
- Operator uploaded three PDFs, "Huge find": `RV2419.pdf` (11-page scan — a Webb Law Firm cease-and-desist letter plus the full federal court filing), `health.pdf` (a 23andMe Reports Summary), and a Herald-Standard news article confirming the case's outcome.
- **The Ford trademark litigation gap — open since page creation — is resolved.** `rick-frank.md`'s Gaps carried "the Ford trademark litigation outcome" as unresolved since 2026-06-23. The two new primary documents close it: Frank's Auto Supply, Inc. (the business's formal legal name; "Auto Supermarket" is the household/retail name) filed suit against Ford Motor Company March 27, 2009 (Case No. 2:09-cv-365, W.D. Pa., Judge Arthur J. Schwab) over Ford's "We Speak Car" ad campaign for the 2010 Fusion/Fusion Hybrid (a $60-80M national campaign, debuted during *American Idol*) infringing Frank's own "We Speak Car" mark (in use since June 2003). Counsel (The Webb Law Firm) sent a cease-and-desist to Ford's General Counsel the same day the complaint was filed but held unserved — filing first, then negotiating — cc'ing Ford's then-VP of Marketing Jim Farley (later Ford's CEO). The suit asked the court to order all of Ford's "We Speak Car" materials destroyed. Judge Schwab closed the case April 17, 2009 on an "amicable resolution," terms undisclosed — three weeks from cease-and-desist to dismissal, against one of the largest advertisers in the country. Filed to `raw/legal/documents/2009-03-27_ford-trademark-cease-desist-and-complaint.md` (+ source PDF) and `raw/legal/documents/2009-04-19_herald-standard-ford-settlement-article.md` (+ source PDF); new "The Ford trademark suit, 2009 — resolved" section added to `rick-frank.md`, Gaps line struck through with a RESOLVED pointer. Per the legal-domain index's own convention (other people's legal events live on their people page, not `wiki/legal/`), no new `wiki/legal/` page was created — Rick's business dispute belongs on his page.
- **The 23andMe `health.pdf` was already on file (byte-identical, confirmed via md5) but its contents had never been extracted** — `wiki/self/lineage/23andme-genomics.md` explicitly carried "no text-layer extraction is possible" and "health report data remains unprocessed" as standing gaps since 2026-07-25. That premise was wrong: the PDF has a real text layer, but a two-column condition/result layout that a naive extraction badly mis-pairs (a real accuracy risk on sensitive health data). `pdftotext -layout` (poppler-utils, installed this pass) preserves the correct column pairing; full extraction filed to `raw/self/ancestry/extracted/health-report-extracted-layout.txt`.
- **Health findings, verified pairing:** only two non-default results across 14+ Health Predisposition and 46+ Carrier Status reports — **Hereditary Thrombophilia: slightly increased risk**, and **Tyrosinemia Type I: variant detected (carrier status)**. Everything else: variants not detected.
- **Exact ancestry-composition percentages, previously flagged as unextracted on `ancestral-dialectic.md` and `23andme-genomics.md` both:** European 99.7% (Northwestern European 78.3% — British & Irish 55.8%, French & German 22.2% — Ashkenazi Jewish 21.4%), plus a **previously undocumented 0.2% Sub-Saharan African (Ghanaian, Liberian & Sierra Leonean) trace signal** with no paper-trail corroboration in the GEDCOM — recorded as a documented gap, not folded into the two-line dialectic frame, since 23andMe's own trace-region confidence at this magnitude is low. Maternal Haplogroup R0, Paternal Haplogroup R-Z93. Neanderthal ">95th percentile of customers" confirmed in the report's own wording, matching the figure already on file.
- Both `wiki/self/lineage/23andme-genomics.md` and `wiki/mind/synthesis/ancestral-dialectic.md` updated — the latter's Gaps section resolved with a dated block rather than silently edited, per the "never bump a date without re-reading the premise" rule, since the dialectic's biological-corroboration claim is now more precisely supported (21.4%/78.3% split roughly tracks the paternal/maternal frame once compared Ashkenazi-vs-non-Ashkenazi rather than a strict 50/50).
- Gates: wiki-lint 456 pages / 0 errors (13 warnings, unchanged pre-existing set) · wiki-connect check 0 errors / 234 warnings (unchanged — no new edges added this pass, existing `component-of`/`evidences` edges cover the updated pages) · wiki-climb check 456 pages, 27 with `synthesizes:`, 0 errors, 0 warnings. `bin/wiki-digest` and `bin/llm-publish` rerun.
- Pages touched: `wiki/people/rick-frank.md`, `wiki/self/lineage/23andme-genomics.md`, `wiki/mind/synthesis/ancestral-dialectic.md`. Raw filed: `raw/legal/documents/2009-03-27_ford-trademark-cease-desist-and-complaint.{md,pdf}`, `raw/legal/documents/2009-04-19_herald-standard-ford-settlement-article.{md,pdf}`, `raw/self/ancestry/extracted/health-report-extracted-layout.txt`.
