...
* **RESUME POINT — this is now the main line of work:**
  1. **Continue the read at `bin/annie-corpus read 2015-11-30`** (403 messages), then December 2015 (**12,000 messages**, the onset flood — several sessions on its own). Update the progress table and the event count on `annie-record.md` every session. Do **not** revert to scripted extraction; the operator has ruled on this and two mechanical attempts failed.
