...
* **Summary:** Operator directive — `/wiki-rewrite` three person pages. First real use of the `wiki-rewrite` skill (previously untested, per the 2026-08-08 governance handoff's resume point #3). All three pages already passed all gates and read as complete; the skill's core premise — "the product is the delta between what the page said and what the corpus says" — held anyway.
* **Findings, in order of value:**
  1. **A fabricated source citation on `jason-bermejo.md`.** The page cited a CSV file for a "Feb 2026, 400+ messages/month" renewal claim. The file does not exist anywhere in `raw/`. `all_imessages_complete_dump.txt` — complete for this handle through its real last message, 2025-03-17 — shows no 2026 activity at all. Removed with a `CORRECTED` block; real figures (292 messages, 2017–2025) restored. The whole page body was also still in pre-STYLE_GUIDE bullet-fragment "dossier shorthand," rewritten to prose.
  2. **`eric-jester.md`'s real date range was off by eight years, and a whole source (220-message Facebook Messenger archive, Jan 2009–Sep 2020) had never been searched for.** Real record: 2009–2025. Inside the Facebook archive: Dan's own, highly granular account of Fran Coldren's death (April 5–6, 2018) — paid $15/hr caregiving, a bedside "sign" test, the moment on camera, inheritance named as his reason for leaving PA the same week she died. This is a **second telling** of the act `fran-death-vigil.md` already carries from a same-night message to Lucie Dobbin, and the two versions genuinely disagree on particulars (blink vs. smile, asleep vs. awake, recovery vs. death). Recorded side by side rather than harmonized — see the page and `fran-death-vigil.md`'s own new section.
  3. **A previously-undated event got a date.** A July 5, 2013, 1:31 AM message to Eric — "dude i got punched in the face by alexis today hahaha" — is the first primary-dated evidence located anywhere for `franki-fireworks-day-2013.md`, whose own Gaps section had called the date unfixed since 2026-08-02. Narrows "early July" to the July 4–5 weekend and adds a physical-altercation detail no other account carries.
  4. **Also found via Eric's thread:** Dan's Oct 2011 Occupy Wall Street participation (predates the wiki's "2020 left turn" framing by nine years — not chased further this pass, flagged for whoever works that synthesis next); a Boston Marathon bombing near-miss ("Kendel was 2 blocks away," April 2013); Eric's own bartending suspension (a thrown lemon, Oct 2011); independent corroboration that Dan's rollerblading-identity years left a durable public trace (a video, referenced May 2013 — no dedicated wiki page for that era exists yet).
  5. **`shelbie-breakiron.md`'s 685-message count survived a re-derivation** that could have gone the other way — the complete dump only shows 471 for that handle. Kept 685 (the per-thread CSV's own direction column reads as reliable), logged the shortfall as an unexplained dump-coverage gap rather than picking silently. A standalone RTF capture supplied a **April 17, 2019** dated message — the earliest primary contact in the corpus, predating the CSV thread by a month and the stated threesome date by twelve days.
  6. **Write-back was most of the actual work.** All three pages declared outbound edges whose targets never got the inverse — CLAUDE.md's most common failure, named explicitly. Wired real inverses (several with load-bearing prose) across eleven neighbor pages, retrofitted two legacy `related:` pages with bare `## Related` footers (`shelbie-annie-threesome-april-2019`, `trinity-st-clair`) to typed `connections:`, and fixed one mismatched edge-type pair (`jamie-mohler`/`jason-bermejo`, `evidenced-by` against `co-occurs` instead of the correct `evidences` pair).
* **Staleness:** one page, `suzanne-frank.md`, went stale off the `fran-death-vigil.md` edit (its own `synthesizes:` list). Re-read: the new material is about Dan's retelling and motive, not Suz's caretaking role. Survives unchanged; RE-CHECKED block added, not a date bump.
* **Gates:** `wiki-lint` 439 pages / 0 errors (13 warnings, pre-existing) · `wiki-connect check` 0 errors / 193 warnings (down from 208 at session start) · `wiki-climb check` 439 pages, 21 with `synthesizes:`, 0 errors, 0 warnings. `bin/wiki-digest` and `bin/llm-publish` rerun.
* **RESUME POINT:**
  1. The Occupy Wall Street finding on `eric-jester.md` (Dan actively participating, Oct 2011) is unintegrated into `wiki/mind/synthesis/2020-left-turn.md` or any ideology page — it complicates a "2020 turn" framing with nine years of prior lead time and deserves a real look, not a footnote.
  2. Dan's childhood rollerblading-identity years (age ~11–17, corroborated independently via Eric Jester's thread) have no dedicated wiki page as of this pass. A capture on this subject may exist or be forthcoming; check before writing one from scratch.
  3. The "J"/"Mawfuckin J" figure in Eric's 2016 Facebook message is unresolved — plausibly Jason Bermejo, not confirmed.
  4. The `wiki-rewrite` skill held up on its first real test — no corrections needed to the skill file itself this pass.
* **Handoff Note:** Pushed to `feat/shelbie-jason-eric-rewrite`. All three gates 0 errors.
