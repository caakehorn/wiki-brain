...
### [2026-08-11] - Session (continued a fifth time): Ford trademark suit resolved, 23andMe health/ancestry percentages extracted
* **Model:** Claude Sonnet 5 (Claude Code, remote) · **Branch:** `claude/rick-correspondence-review-rb6mx2` (PR #107 from the prior round merged; this round continues on the same session-designated branch, restarted from latest `main` per the merged-PR protocol)
* **Trigger:** Operator uploaded three PDFs, "Huge find": `RV2419.pdf` (11-page scan — Webb Law Firm cease-and-desist letter + full federal court filing), `health.pdf` (a 23andMe Reports Summary), and a Herald-Standard article.
* **Environment note:** this environment had no PDF-rendering/parsing tooling installed by default. Fixed via `apt-get update && apt-get install -y poppler-utils` (enables the Read tool's native page-image rendering for scanned PDFs, and `pdftotext`) and `pip install pdfminer.six`. `pypdf` is broken in this sandbox (`_cffi_backend`/cryptography Rust-panic on load) — do not use it here; `pdftotext -layout` (not a plain/no-layout extraction) is required for any multi-column PDF to avoid mis-pairing label/value columns.
* **Ford trademark litigation gap — resolved, open since page creation (2026-06-23).** Frank's Auto Supply, Inc. (the formal legal name; "Auto Supermarket" is the retail name) sued Ford Motor Company March 27, 2009 (2:09-cv-365, W.D. Pa.) over Ford's "We Speak Car" 2010-Fusion ad campaign ($60-80M, debuted on *American Idol*) infringing FAS's own "We Speak Car" mark (in use since 2003). Filed suit and sent a cease-and-desist the same day (cc: Jim Farley, then Ford's VP Marketing, later its CEO). Settled amicably, case closed by court order April 17, 2009 — three weeks, terms undisclosed. Filed to `raw/legal/documents/2009-03-27_ford-trademark-cease-desist-and-complaint.{md,pdf}` and `2009-04-19_herald-standard-ford-settlement-article.{md,pdf}`; new section on `rick-frank.md`, Gaps line resolved with a pointer rather than deleted. No new `wiki/legal/` page — per that index's own convention, other people's legal events live on their people page.
* **`health.pdf` was already filed (byte-identical, md5-confirmed) but never actually extracted** — `23andme-genomics.md` had carried "no text-layer extraction is possible" / "health report data remains unprocessed" as standing gaps since 2026-07-25. Both were wrong: it has a real text layer, just a two-column layout a naive extraction mis-pairs. `pdftotext -layout` fixed it. Health findings: only **Hereditary Thrombophilia (slightly increased risk)** and **Tyrosinemia Type I (carrier)** returned anything but "not detected" across 60+ reports. Ancestry composition, previously flagged unextracted on two pages: European 99.7% (NW European 78.3% — British & Irish 55.8%, French & German 22.2% — Ashkenazi Jewish 21.4%), plus a **new, previously-undocumented 0.2% Sub-Saharan African trace signal** with no paper-trail corroboration — recorded as an open gap on `23andme-genomics.md`, not folded into `ancestral-dialectic.md`'s two-line frame (that page's Gaps section updated with a dated RESOLVED block instead of a silent edit, per the staleness discipline, since its biological-corroboration claim is now more precisely supported).
* **Gates:** wiki-lint **456 pages / 0 errors** (13 warnings, unchanged) · wiki-connect check **0 errors** (234 warnings, unchanged — no new edges needed, existing edges already cover the updated pages) · wiki-climb check **456 pages, 27 with `synthesizes:`, 0 errors, 0 warnings**. `bin/wiki-digest` and `bin/llm-publish` rerun.
* **RESUME POINT:**
  1. The 0.2% Sub-Saharan African trace signal on `23andme-genomics.md` has no explanation anywhere in the corpus — worth a `family-tree.md` cross-check if a future pass wants to chase it, but not urgent given 23andMe's own low confidence at trace-region magnitude.
  2. Whether the Hereditary Thrombophilia finding has ever come up in a medical context is unrecorded — flagged as a gap, not investigated further this pass (no source pointed anywhere to look).
  3. RV2419.pdf's Complaint document is cut off at paragraph 25 (Count II) in the scan provided — the prayer for relief and signature page were not included; if a more complete filing ever surfaces, reconcile against the Herald-Standard article's mention of a "destroy all literature" request.
  4. Prior resume points (Syd's identity, the Sept 2023 Rick/Lisa "wedding," two undecoded Aug 2025 photo attachments, the two "Irreversibility Firewall" predictions needing a real falsifier hunt) still stand — untouched this pass.
* **Handoff Note:** All three gates 0 errors. PR #107 (totality-themes) merged during this session — separate matter, no action needed. Committing and pushing this round now; draft PR to follow.
