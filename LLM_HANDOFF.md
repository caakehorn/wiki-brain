...
### [2026-09-04c] - Session: the corpus had his monologue and none of his dialogue (Claude Opus 5)
...
**`wiki/health/intake-ledger.md`** — every unit, every event, every correction,
per-substance figures with coverage, and a bar chart of events per hour.
Generated by **`bin/intake page`**, gated on drift in `bin/intake check`.

**Read these before touching it:**

1. **It is generated. Never hand-edit it** — `bin/intake check` fails on drift
   and the fix is always to rerun the tool. It runs **first** in
   `bin/wiki-check`'s generator list, ahead of `wiki-digest` and `llm-publish`,
   because it writes into `wiki/` and both of those read `wiki/`.
2. **It is evidence, not a claim, and a finding must never be written onto it.**
   The page is overwritten on every write and would lose it. Findings go on
   `wiki/health/cocaine.md` and `wiki/health/chemical-architecture.md` through
   the normal operations, cited to unit ids. `intake/README.md` and `CLAUDE.md`
   both said "it is not a wiki page and does not become one automatically" —
   updated in both, because that sentence is now false and the rule under it is
   not.
3. **The per-unit rate is withheld on purpose, not forgotten.**
   `report`'s `Rate of consumption ... g / day` extrapolates a unit's quantity
   across a full day from however long the unit lived (1.89 g/day for 0.75 g
   actually consumed). A test asserts no per-day *value* can reach the page.
   Do not "fix" its absence.
4. **Adding a field to the page means adding a test.** Two defects landed in the
   first render and both were invisible rather than loud: a bare `"Daily"` inside
   a double-quoted YAML claim, which breaks frontmatter silently, and correction
   rows printing fields that had not changed. `TestWikiPage` pins both.

**Also updated:** `wiki/health/index.md` (the page was an orphan without it) and
the master `index.md`'s health count, 4 → 5 — `tests/test_lint_gates.py`
`test_real_index_is_current` catches that one, and it is the reason to run the
whole suite rather than just `test_intake` after adding a page.

**Unchanged and worth checking stayed unchanged:** 90 stale premises, the same
number as before this pass and the one before it. The 95 obligations are 90
stale + 5 skill candidates, two of which this session parked.
