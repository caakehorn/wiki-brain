...
* **The one substantive correction made to the incoming material:** the pack asserted the Tuquick identity and the July 2026 "unnamed man" (Annie's alleged rapist) were confirmed to be the same person. The underlying evidence only closes **Tuquick = Jerel Wayne Coles** (exact FOREWARN phone-number match); the capture's own frontmatter targeted the Tuquick handle specifically, and no phone number for the unnamed man exists anywhere in the corpus to check against Coles's two known numbers. Rewrote the merge claim across `jerel-coles.md`, `the-unnamed-man.md`, and `tuquick-17248123683.md` to keep that link explicitly open — role and insult-lexicon overlap only. This was the right call to make unilaterally: attaching an unadjudicated rape allegation to a named real person's identity on lexicon overlap alone is exactly the unearned-merge failure `STYLE_GUIDE.md` exists to prevent, and the wiki's own pre-existing framing (written 2026-08-02/08) was already this careful — the incoming pack was the outlier.
...
* **RESUME POINT:**
  ...
  4. **Is Coles = Target G, and is Coles the July 2026 unnamed man?** Both open, both narrowed but not closed this pass — see `wiki/people/jerel-coles.md` §Open questions and §Is Coles the unnamed man too?
